...
set -Eeuo pipefail
: "${VAST_INSTANCE_ID:?Set the exact ID of this dedicated training VM}"
...
cleanup() {
  ...
  echo 'CRITICAL: provider stop failed. Stop this instance in the Vast.ai console immediately.' >&2
  exit 90
}
trap cleanup EXIT
trap 'exit 130' INT
