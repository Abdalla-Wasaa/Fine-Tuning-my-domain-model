...
def main():
    ...
    require_llama_memory(args.precision)
    ...
    model = AutoModelForCausalLM.from_pretrained(manifest['config']['base_model'],
        revision=manifest['base_revision'], torch_dtype=getattr(torch, args.precision), device_map='cpu', low_cpu_mem_usage=True)
    ...
    write_json(args.output / 'merge_manifest.json', {'base_model': manifest['config']['base_model'],
        'precision': args.precision, 'base_revision': manifest['base_revision'], 'training_manifest_sha256': sha256(args.adapter / 'run_manifest.json'),
